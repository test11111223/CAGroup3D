...
def boxes_bev_iou_cpu(boxes_a0, boxes_b0):
    """
    Args:
    ...
    """
    # ME = CPU, PCDET = CUDA
    boxes_a = boxes_a0.clone().detach().cuda()
    boxes_b = boxes_b0.clone().detach().cuda()
...
def boxes_iou_bev(boxes_a0, boxes_b0):
    """
    Args:
        boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
        boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
    ...
    Returns:
        ans_iou: (N, M)
    """
    assert boxes_a0.shape[1] == boxes_b0.shape[1] == 7
...
def boxes_iou3d_gpu(boxes_a0, boxes_b0):
    """
    Args:
        boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
        boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
...
def nms_gpu(boxes0, scores0, thresh0, pre_maxsize=None, **kwargs):
    """
...
def nms_normal_gpu(boxes0, scores0, thresh0, **kwargs):
    """
    :param boxes: (N, 7) [x, y, z, dx, dy, dz, heading]
    :param scores: (N)
    :param thresh:
